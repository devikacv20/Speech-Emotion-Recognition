...
datapath = "R:\\Mini\\D\\1D-Speech-Emotion-Recognition-master\\data\\wav"
...
def countclasses(fnames):
    ...
    for name in fnames:
        if len(name) > 5 and name[5] in classes:  
            dict[name[5]] += 1
    return dict


def data1d(path):
    fnames = os.listdir("R:\\Mini\\D\\1D-Speech-Emotion-Recognition-master\\data\\wav")
    ...
    for name in fnames:
        if len(name) > 5 and name[5] in classes:  
            sig, fs = librosa.load("R:/Mini/D/1D-Speech-Emotion-Recognition-master/data/wav" + '/' + name, sr=16000)
...
def load_data():
	x_tr,y_tr,x_t,y_t,x_v,y_v = data1d("R:\\MiniProject\\D\\1D-Speech-Emotion-Recognition-master\\data\\lablaut")
